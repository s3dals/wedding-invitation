...
export const gif = (() => {
    ...
    const template = (uuid) => {
        ...
        return `
        <label for="gif-search-${uuid}" class="form-label my-1"><i class="fa-solid fa-photo-film me-2"></i>${content.get('gif_label') ?? 'Gif'}</label>
        ...
        <div class="d-flex mb-3" id="gif-search-nav-${uuid}">
            ...
            <input dir="auto" type="text" name="gif-search" id="gif-search-${uuid}" autocomplete="on" class="form-control shadow-sm rounded-4" placeholder="${content.get('gif_search_placeholder') ?? 'Search for a GIF on Tenor'}" data-offline-disabled="false">
        </div>
